...
export class GoogleAPI {
  ...
  static async createMeeting({ input }) {
    ...
    try {
      ...
      const { google_calendar_id } = await GoogleModel.getCalendarId({ input })
      const { google_refresh_token } = await GoogleModel.getTokens({ input });
      ...
      console.log(result.data)
      return { code: "GOOGLE_MEETING_CREATED", data: result.data }
    } catch (e) {
      try {
        if (e.response.data.error == "invalid_grant") {
          return { code: "ER_GOOGLE_INVALID_GRANT", data: e }
          ...
          return { code: "ER_GOOGLE_CALENDAR_NOT_FOUND", data: e }
        } else return { code: "ER_UNKNOWN_WHEN_CREATING_GOOGLE_MEETING", data: e }
      } catch {
        return { code: "ER_UNKNOWN_WHEN_CREATING_GOOGLE_MEETING", data: e }
      }
    }
  }
}